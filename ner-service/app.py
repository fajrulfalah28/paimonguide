...
def word2features(sentence: list[str], pos_tags: list[str], index: int, gazetteer_names: set[str], gazetteer_tokens: set[str], sentence_lookup: set[str]) -> dict[str, object]:
    word = sentence[index]
    normalized = normalize_token(word)
    postag = pos_tags[index]
    in_token_set = normalized in gazetteer_tokens
    in_full_name = normalized in gazetteer_names
    in_sentence_gazetteer = any((phrase in gazetteer_names for phrase in sentence_lookup if normalized and normalized in phrase.split()))
    features = {'bias': 1.0, 'word': word, 'word.lower()': word.lower(), 'word.normalized': normalized, 'word.isupper()': word.isupper(), 'word.istitle()': word.istitle(), 'word.isdigit()': word.isdigit(), 'word.has_hyphen': '-' in word, 'word.has_apostrophe': "'" in word, 'word.has_colon': ':' in word, 'word.prefix2': normalized[:2], 'word.prefix3': normalized[:3], 'word.suffix2': normalized[-2:], 'word.suffix3': normalized[-3:], 'postag': postag, 'postag[:2]': postag[:2], 'is_sentence_start': index == 0, 'is_sentence_end': index == len(sentence) - 1, 'is_location_connector': normalized in LOCATION_CONNECTORS, 'matches_alias': normalized in ALIAS_MAP, 'in_alias_expansion': any((normalized in expansion.split() for expansion in ALIAS_MAP.values())), 'is_in_genshin_map': in_token_set or in_full_name, 'gazetteer_word_overlap': in_token_set, 'gazetteer_phrase_overlap': in_sentence_gazetteer}
    if index > 0:
        prev_word = sentence[index - 1]
        prev_norm = normalize_token(prev_word)
        features.update({'-1:word.lower()': prev_word.lower(), '-1:word.normalized': prev_norm, '-1:postag': pos_tags[index - 1], '-1:postag[:2]': pos_tags[index - 1][:2], '-1:is_connector': prev_norm in LOCATION_CONNECTORS})
    else:
        features['BOS'] = True
    if index < len(sentence) - 1:
        next_word = sentence[index + 1]
        next_norm = normalize_token(next_word)
        features.update({'+1:word.lower()': next_word.lower(), '+1:word.normalized': next_norm, '+1:postag': pos_tags[index + 1], '+1:postag[:2]': pos_tags[index + 1][:2], '+1:is_connector': next_norm in LOCATION_CONNECTORS})
    else:
        features['EOS'] = True
    if index > 1:
...
def sent2features(sentence: list[str], gazetteer_names: set[str], gazetteer_tokens: set[str]) -> list[dict[str, object]]:
    ...
    return [word2features(sentence, pos_tags, idx, gazetteer_names, gazetteer_tokens, sentence_lookup) for idx in range(len(sentence))]
...
def tokenise_for_overlap(text: str) -> list[str]:
    ...
    rough_tokens = re.split('[\\s:\\-]+', normalized_text)
    cleaned_tokens = []
...
def score_overlap_candidate(raw_span: str, candidate_name: str) -> dict[str, object]:
    ...
    return {'name': candidate_name, 'score': round(overlap_ratio, 3), 'overlap_count': overlap_count, 'overlap_ratio': round(overlap_ratio, 3), 'overlap_tokens': overlap_tokens, 'candidate_token_count': len(candidate_tokens), 'reason': explain_candidate_reason(overlap_count, overlap_ratio, overlap_tokens, 'overlap')}
...
def rank_overlap_candidates(raw_span: str, gazetteer_names: set[str], max_candidates: int=MAX_RESOLVER_CANDIDATES) -> list[dict[str, object]]:
    ...
    scored_candidates.sort(key=lambda item: (item['overlap_count'], item['overlap_ratio'], -item['candidate_token_count'], -len(item['name'])), reverse=True)
    return scored_candidates[:max_candidates]

def build_exact_entity(match_name: str) -> dict[str, object]:
    return {'raw_span': match_name, 'resolved_name': match_name, 'method': 'exact', 'confidence': 1.0, 'ambiguous': False, 'candidates': [{'name': match_name, 'score': 1.0, 'reason': explain_candidate_reason(0, 1.0, [], 'exact')}], 'fuzzy_corrections': []}

def build_alias_entity(raw_span: str, resolved_name: str) -> dict[str, object]:
    return {'raw_span': raw_span, 'resolved_name': resolved_name, 'method': 'alias', 'confidence': ALIAS_CONFIDENCE, 'ambiguous': False, 'candidates': [{'name': resolved_name, 'score': ALIAS_CONFIDENCE, 'reason': explain_candidate_reason(0, ALIAS_CONFIDENCE, [], 'alias')}], 'fuzzy_corrections': []}
...
def build_overlap_entity(raw_span: str, overlap_candidates: list[dict[str, object]], method: str, confidence: float, ambiguous: bool, fuzzy_corrections: list[dict[str, object]]) -> dict[str, object]:
    return {'raw_span': normalize_name(raw_span), 'resolved_name': overlap_candidates[0]['name'], 'method': method, 'confidence': round(confidence, 3), 'ambiguous': ambiguous, 'candidates': overlap_candidates, 'fuzzy_corrections': fuzzy_corrections}
...
def resolve_span_to_canonical(raw_span: str, gazetteer_names: set[str], gazetteer_tokens: set[str], allowed_fuzzy_tokens: set[str] | None=None) -> dict[str, object]:
    ...
    if normalized_span in ALIAS_MAP:
        ...
        if len(tokenise_for_overlap(normalized_span)) == 1 and len(overlap_candidates) > 1 and (abs(overlap_candidates[0]['score'] - overlap_candidates[1]['score']) <= AMBIGUITY_MARGIN):
            alias_entity['ambiguous'] = True
            alias_entity['candidates'] = overlap_candidates
        return alias_entity
    overlap_candidates = rank_overlap_candidates(normalized_span, gazetteer_names)
    ...
    if corrected_span and corrected_span != normalized_span and fuzzy_corrections:
        if corrected_span in ALIAS_MAP:
            ...
            alias_entity['candidates'] = [{'name': alias_entity['resolved_name'], 'score': alias_entity['confidence'], 'reason': 'Resolved after fuzzy token correction and alias lookup.'}]
            return alias_entity
    ...
    return {'raw_span': normalized_span, 'resolved_name': normalized_span, 'method': 'raw', 'confidence': round(final_candidates[0]['score'], 3) if final_candidates else 0.0, 'ambiguous': False, 'candidates': final_candidates, 'fuzzy_corrections': fuzzy_corrections if 'fuzzy_corrections' in locals() else []}
...
def resolve_query_locations(raw_text: str, model, gazetteer_names: set[str], gazetteer_tokens: set[str]) -> dict[str, object]:
    ...
    if not tokens:
        return {'query': raw_text, 'tokens': [], 'crf_tags': [], 'crf_spans': [], 'exact_matches': [], 'alias_matches': [], 'fallback_token_spans': [], 'entities': [], 'resolved_names': []}
...
def print_query_resolution(query_text: str, title: str) -> None:
    resolution = resolve_query_locations(query_text, results['model'], results['gazetteer_names'], results['gazetteer_tokens'])
